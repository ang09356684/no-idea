...
> 註：目前只設了 **Production** 環境變數。**Preview**（分支/PR 預覽部署）未設——preview 站會優雅降級（可瀏覽、登入停用）。若要 preview 也能登入，到 Vercel 後台 Settings → Environment Variables 對 Preview 補上同一組即可。
...
**GitHub 自動部署（已啟用）**：專案已在 Vercel 後台 **Connect Git Repository** 連結 `ang09356684/no-idea`。
- `git push` 到 **master** → **自動部署到 production**（線上自動更新）
- 其他分支 / PR → 自動 **preview 部署**（獨立網址）

**手動部署**（不想 push 也可）：本機跑 `vercel --prod`。
